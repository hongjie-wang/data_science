...
rm(list = ls())
library(HydeNet)
# notice in this case, bet is a node determined by forecast
# in other words, we simply executing a strategy and are interested in utility

net<-HydeNetwork(~+weather+result|weather+forecast|weather+bet|forecast+U|bet*result)
...
net<-setNode(net,bet,nodeType = "determ",define=fromFormula(),
             nodeFormula = bet~ifelse(forecast=="rainy",1,0))
...
trackedVars<-c("weather","result","forecast","bet","U")
...
evidence=list(forecast="cloudy")
compiledNet<-compileJagsModel(net,data=evidence,n.chain=3,n.adapt=5000)
post<-HydeSim(compiledNet,variable.names =trackedVars,n.iter=10000)
mean(post$U)

evidence=list(forecast="sunny")
compiledNet<-compileJagsModel(net,data=evidence,n.chain=3,n.adapt=5000)
post<-HydeSim(compiledNet,variable.names =trackedVars,n.iter=10000)
mean(post$U)

evidence=list(forecast="rainy")
compiledNet<-compileJagsModel(net,data=evidence,n.chain=3,n.adapt=5000)
post<-HydeSim(compiledNet,variable.names =trackedVars,n.iter=10000)
mean(post$U)

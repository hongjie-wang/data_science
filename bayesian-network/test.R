...
net<-HydeNetwork(~marketing+cost1|marketing
                 ...
                 +avg.capacity+capacity|avg.capacity
                 +cost4|avg.capacity
...
net<-setNode(net,value.per.customer,nodeType = "dnorm",mean=0.02,tau=2000)
...
#net<-setDecisionNodes(net,avg.capacity)
...
#evidence=NULL
evidence=list(avg.capacity=700)
trackedVars<-c("marketing","modeling","resp.rate","cost1","responders",
               ...
               "avg.capacity","capacity","cost4","serv.quality",
...
for (i in seq(200,1000,10)){
  ...
  results=cbind(avg.capacity=i,serv.quality=mean(post$serv.quality),profit=mean(post$total.profit))

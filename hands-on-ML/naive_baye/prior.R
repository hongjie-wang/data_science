...
y=rbinom(n,1,prob)
population<-data.frame(x=x,y=as.factor(y))

split_strat<-initial_split(population,prop=0.5,strata="y")

mydata<-training(split_strat)

future_data<-testing(split_strat)

# in future data,the rate is around 50% 
table(future_data$y)

# we got a downsized sample very inbalanced
# notice this is response based sampling
